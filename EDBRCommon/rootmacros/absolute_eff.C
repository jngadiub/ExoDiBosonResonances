...
*/
	...
	TH1F * h1b = new TH1F ("Efficiency","Efficiency",17,550,2250);
		...
		for(int j=0;j<entries;j++)
		{
			...
			for(int ivec=0;ivec<nCands;ivec++){
				...
				if(filled==0)//cout the event as pass if any candidate pass all the cuts
				{
					...
					if((nxjCut > 0) and 
					   (nXjets[ivec]!=nxjCut))continue;// 1 jet candidate
	...
	TF1 *fitFunc = new TF1("fitPoly2","tanh([0]*(x-[1]))*([2]+[3]*x)",580,2050);
	...
	h1b->Draw("P");
	fitFunc->Draw("Lsame");
	c2->SaveAs("efficiencyAndFit.pdf");
