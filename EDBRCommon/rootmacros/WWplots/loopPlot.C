...
void loopPlot(){
	...
	double lumiValue = 0;  
	if(wantMuons)lumiValue = 19706.255;
	if(wantElectrons)lumiValue = 19683.255;
	...
	//std::string pathToTrees="/afs/cern.ch/work/s/shuai/public/diboson/trees/productionv12/ttbarcontrol/";

	/// Path to wherever you want to put the histograms (figures) in.
	// std::string outputDir = "./plots_WW_ele_fullrange_btagCM_ALLP_v12_ratio_v1";
	//std::string outputDir = "./plots_WW_test";
	std::string outputDir = "./plots_WW_both_signal_bvetoM_ALLP_v12_noratio_v2";
	...
	{
		mcLabels[0]="SingleTop_xww";
		mcLabels[1]="VV_xww";
		mcLabels[2]="WJetsPt100_xww";
		mcLabels[3]="TTBARpowheg_xww";
	}
