		...
		for(int ivec=0;ivec<nCands;ivec++){
			...
			if(eventPassesCut(ivec, 80, 20)){
				...
				if(isZZchannel_==0)//WW channel, veto second loose lepton
				{
					...
					//cut from fermilab
					if(isTTcontrol_==true||deltaR_LJ>1.57 && deltaPhi_JMET>2. && deltaPhi_JWL>2.);
					else continue;
					...
					//n b jet cut
					if(isTTcontrol_==false&&nbtagsM[ivec]==0) ;//b veto cut
					else if(isTTcontrol_==true&&nbtagscleanM[ivec]>=1) ;//b cut - ttbar control region - to sync we use clean M
					else continue;

					//nsubjettiness or purity
					if(wantPurity_==1&&nsubj21[ivec]<0.5) ;//HP
					else if(wantPurity_==0&&nsubj21[ivec]>0.5&&nsubj21[ivec]<0.75);//LP
					else if(wantPurity_==-1);//ALLP
					else continue;
