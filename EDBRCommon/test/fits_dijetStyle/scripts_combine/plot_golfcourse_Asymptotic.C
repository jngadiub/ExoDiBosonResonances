...
void plot_golfcourse_Asymptotic(bool unblind){
  ...
  string xsect_file_th="../data/xsect_BulkG_ZZ_c0p2_xsect_in_pb.txt";
  if(!isZZChannel)xsect_file_th="../data/xsect_BulkG_WW_c0p2_xsect_in_pb.txt";
  ...
  if(!isZZChannel)xsect_file_interpol2="../data/xsect_BulkG_WW_c0p5_xsect_in_pb.txt";
  ...
  */
  ...
  leg->AddEntry(grthSM10, "#sigma_{TH} x BR(G #rightarrow "+VV+"), #tilde{k}=0.50", "L");// #rightarrow 2l2q
  leg->AddEntry(grthSM, "#sigma_{TH} x BR(G #rightarrow "+VV+"), #tilde{k}=0.20", "L" );// #rightarrow 2l2q
  leg->Draw();
