#!/usr/bin/env python
# original by Colin in CMGTools/Production/scripts/cmsBatch.py
...
eval `scramv1 ru -sh`
cd -
### $LS_SUBCWD is defined only when you execute it on the LXBatch
### and it is the directory from which it is submitted
### the parallel job (i.e.: ... logs/Summer12/<sampleNAme/Job_0/)
cp -rf $LS_SUBCWD .
ls
cd `find . -type d | grep /`
echo 'running'
...
cmsStage -f $file $fullFileName
#this does cmsStage, but with retries (5 retries with increasing waiting time)
...
class MyBatchManager( BatchManager ):
   '''Batch manager specific to cmsRun processes for EXO-VV searches.''' 
